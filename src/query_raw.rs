...
impl RawQueryResult {
    pub(crate) fn fetch_cell(&self, row: usize, column: usize) -> Option<&[u8]> {
        unsafe {
            ...
            let data: *mut u8 =
                libpq::PQgetvalue(self.result, row as i32, column as i32) as *mut u8;
...
impl RawQueryResult {
    ...
    pub(crate) fn PQgetlength(&self, row: usize, column: usize) -> usize {
        (unsafe { libpq::PQgetlength(self.result, row as i32, column as i32) } as usize)
    }

    pub(crate) fn PQntuples(&self) -> usize {
        (unsafe { libpq::PQntuples(self.result) } as usize)
    }
